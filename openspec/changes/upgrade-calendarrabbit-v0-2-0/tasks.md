...
- [ ] 1.3 push 後、`kubectl get secret calendarrabbit-secret -n calendarrabbit -o jsonpath='{.data}'` に 4 キー（`DB_PASSWORD`・`CLAUDE_API_KEY`・`DEEPSEEK_API_KEY`・`SEARCH_API_KEY`）が揃っていることを確認する
...
- [ ] 3.3 push → ArgoCD 同期後、`kubectl get pods -n calendarrabbit` で backend/frontend Pod が Running になり、`CreateContainerConfigError` が発生していないことを確認する
