...
SPF レコードはさくらの送信サーバを許可する形式（`v=spf1 a:<sakura-server> mx ~all`）であること。

#### Scenario: SPF レコードが存在する

- **WHEN** `dig TXT i-tk.dev` を実行する
- **THEN** `v=spf1` で始まる TXT レコードが返され、さくらのサーバホスト名が含まれる
...
### Requirement: DKIM TXT レコードが設定されること

さくらが提供するセレクタ名に対応する DKIM TXT レコードが Cloudflare に登録されていなければならない（SHALL）。
セレクタ名は `var.sakura_dkim_selector` として変数化され、TXT 値は `var.sakura_dkim_txt` として外部化されていること。

#### Scenario: DKIM レコードが存在する

- **WHEN** `dig TXT <selector>._domainkey.i-tk.dev` を実行する
- **THEN** `v=DKIM1` で始まる TXT レコードが返される
